...
<head>
	...
	<script src = "js/adminMainPage.js"></script>
</head>
<body>
	...
	<c:choose>
		<c:when test="${requestScope.isReadingRoomInfo eq true}">
			<button type = "button" id = "readingroom" style = "opacity: 1" class = "change-button-style">
				<fmt:message bundle="${loc}" key="local.reading_room_orders" var="reading_room_orders" />
				<c:out value = "${reading_room_orders }"/>
			</button>
			
			<button type = "button" id = "home"  style = "opacity: 0.5" class = "change-button-style-second">
				<fmt:message bundle="${loc}" key="local.home_orders" var="home_orders" />
				<c:out value = "${home_orders }"/>
			</button>
		</c:when>
		<c:when test="${requestScope.isReadingRoomInfo eq false}">
			<button type = "button" id = "readingroom" style = "opacity: 0.5" class = "change-button-style">
				<fmt:message bundle="${loc}" key="local.reading_room_orders" var="reading_room_orders" />
				<c:out value = "${reading_room_orders }"/>
			</button>
			
			<button type = "button" id = "home"  style = "opacity: 1"  class = "change-button-style-second">
				<fmt:message bundle="${loc}" key="local.home_orders" var="home_orders" />
				<c:out value = "${home_orders }"/>
			</button>
		</c:when>
        ...
        <tbody id = "insertBody">
			<c:forEach var="elem" items="${list}" varStatus="status">
					<tr>
						...
						<td>
							<button type = "button" id = "${elem.id}" class="btn add_button">
